...
def get_authenticated_service(args):
  ...
  with open("youtube-v3-discoverydocument.json", "r") as f:
    doc = f.read()
    return build_from_document(doc, http=credentials.authorize(httplib2.Http(proxy_info = httplib2.ProxyInfo(socks.PROXY_TYPE_HTTP, 'localhost', 1080))))
...
def get_comments(youtube, parent_id):
  ...
  while 'nextPageToken' in results:
    ...
    temp_items.extend(results["items"])
    time.sleep(0.3)

  for item in temp_items:
    author = item["snippet"]["authorDisplayName"]
    text = item["snippet"]["textDisplay"]
    channel_id = item["snippet"]["authorChannelId"]["value"] 
    # print ("Comment by %s: %s" % (author, text))
    print("Comment by %s: %s" % (author.encode('utf-8'), channel_id.encode('utf-8')))
...
def get_comment_threads(youtube, video_id):
  ...
  for item in temp_items:
    ...
    if item["snippet"]["totalReplyCount"] > 0:
      ...
      replie_items = get_comments(youtube, item_id)
      all_items.extend(replie_items)
...
def get_all_comments(youtube, video_id):
  all_results = []
  results = get_comment_threads(youtube, video_id)
